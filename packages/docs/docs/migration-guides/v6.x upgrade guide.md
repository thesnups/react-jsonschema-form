...
- `@snups/rjsf-daisyui`: This is new theme based on the `daisyui` toolkit
- `@snups/rjsf-mantine`: This is new theme based on the `mantine` toolkit
- `@snups/rjsf-primereact`: This is a new theme based on the `PrimeReact` toolkit
- `@snups/rjsf-react-bootstrap`: This is rename of the `bootstrap-4` theme with an upgrade to the latest version of `react-bootstrap`
- `@snups/rjsf-shadcn`: This is new theme based on the `shadcn` toolkit
...
#### @snups/rjsf-semantic-ui DEPRECATED
...
This new template was created to extract styling applied to the `MultiSchemaField` component in the `@snups/rjsf-core` package.

If you have styled your form using the following classNames and you do NOT use the `@snups/rjsf-core` theme, you may need to adjust your styles, as they may have been removed from your theme: `panel`, `panel-default`, `panel-body`, `form-group`.
...
The `getMatchingOption()` function in `@snups/rjsf-utils` was removed. Use the `getFirstMatchingOption()` function instead.
I.e.:

```ts
import { getFirstMatchingOption, RJSFSchema } from '@snups/rjsf-utils';
...
```ts
import { createSchemaUtils, RJSFSchema } from '@snups/rjsf-utils';
import validator from '@snups/rjsf-validator-ajv8';
...
The `mergeValidationData()` function from `@snups/rjsf-utils` was removed. Use the `validationDataMerge()` function instead.
I.e.:

```ts
import { validationDataMerge, ValidationData, ErrorSchema } from '@snups/rjsf-utils';
...
```ts
import { toErrorList } from '@snups/rjsf-utils';
...
from `@snups/rjsf-utils` instead. I.e.:

```ts
import { validationDataMerge, ErrorSchema, RJSFValidationError, toErrorList } from '@snups/rjsf-utils';
...
The constant `RJSF_ADDITONAL_PROPERTIES_FLAG` was removed from `@snups/rjsf-utils`. Use the `RJSF_ADDITIONAL_PROPERTIES_FLAG`
constant instead. I.e.:

```ts
import { RJSF_ADDITIONAL_PROPERTIES_FLAG } from '@snups/rjsf-utils';
```
...
```ts
import { UiSchema, RJSFSchema } from '@snups/rjsf-utils';
...
`MultiSchemaField` component in the `@snups/rjsf-core` package, and moved into the `core` theme's `MultiSchemaFieldTemplate`
...
The following new types were added to `@snups/rjsf-utils`:
...
Three new and three formerly internally private utility functions are available in `@snups/rjsf-utils`:
...
Three new validator-based utility functions are available in `@snups/rjsf-utils`:
